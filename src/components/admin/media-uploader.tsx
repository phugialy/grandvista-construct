  ...
  return (
    <form className="border border-ink/12 bg-white p-6">
      ...
      <p className="mt-3 leading-7 text-steel">
        The portal prepares oversized images before upload and blocks files that are too heavy for the website.
      </p>
      ...
      <label className="mt-6 grid cursor-pointer place-items-center border border-dashed border-ink/24 bg-warm-white p-8 text-center hover:border-brand-red">
        ...
        <span className="mt-2 text-sm font-bold text-steel">JPG, PNG, WebP, MP4, WebM, or MOV</span>
        <input
          ref={inputRef}
...
async function prepareFile(file: File) {
  ...
  if (!file.type.startsWith("image/") || file.type === "image/heic" || file.size < 1400 * 1024) {
    return file;
  }
