...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!post) {
    return {
      title: "Post Not Found | Grandvista",
    };
  }

  const title = post.seo_title ?? `${post.title} | Grandvista Community`;
  const description =
    post.seo_description ?? post.excerpt ?? "Commercial construction coverage from Grandvista Construction.";
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://grandvista-construction.com/community/${post.slug}`,
...
export default function CommunityPostPage({ params }: PageProps) {
  return (
    <Suspense fallback={null}>
      <CommunityPostContent params={params} />
    </Suspense>
  );
}

async function CommunityPostContent({ params }: PageProps) {
  await connection();
  const { slug } = await params;
  const post = await getPublishedBlogPostBySlug(slug);
      ...
      <section className="bg-ink text-white">
        <div className="section-shell py-16">
          ...
          <div className="mt-10 grid gap-10 lg:grid-cols-[0.92fr_1.08fr] lg:items-end">
            <div>
              <p className="eyebrow">Grandvista Community</p>
      ...
      <article className="section-shell py-16">
        <div className="max-w-3xl">
          {paragraphs.length > 0 ? (
            ...
            <p className="text-lg leading-9 text-steel">This post is being prepared.</p>
          )}
